 ...
 *@since  o9n 4_9_i$
 */
public class BaseSvs extends BaseHibernateDAO2{
...
//	o9o();
	...
	private static void oaa() {
		...
		List li=new BaseSvs(). findByPropertyss(m,cls);
	 	 core.print_wzFmt(li);
	    ...
	    //	org.apache.taglibs.standard.tlv.JstlCoreTLV
			...
			for (Field fld : flds) {
				try {
					...
					if (fldName.equals("trueDownFlag")) {
						System.out.println("dbg");
					}
					if (fldName.equals("dsId")) {
						System.out.println("dbg");
					}
...
//			 		sql.setCls(cls). addWhereExp(fld);
		...
		//	c.addOrder(Order.desc("materialId"));
			...
			//todox oaL hb bug if(set firstResult not 0(zero), getlist haosyeo shaoleig pagesize (10) rec";
	 		int pageSize = PageUtil.getPageSize(QueryPropertyssMap.get("pagesize"));
	 		int startIndexFirstResult = PageUtil.getStartIndexFirstResult( QueryPropertyssMap.get("page_page"),pageSize);
			c.setFirstResult(0); 		 	//startIndexFirstResult
			c.setMaxResults(999); //should be pageSize 
			String critia_str = c.toString();
			filex.saveLog(critia_str, "c:\\queryCritia");
			List  list = c.list();
			
			//	this.req.getSession().setAttribute("", list);
		  	threadLocal_rowsCount.set(list.size()+0);   //add startIndexFirstResult 
	...
	private boolean isIngroreFld(Field fld) {
		 ...
		 */
		public BaseSvs setSaveObjClass(Class<?> saveObjClass) {
...
public static boolean isInIde=false;
	protected void expLiAddExp(final String fldName, String op,
			...
			try {
				...
				for (String id_int : a) {
					...
					try {
						...
						GvDownloadTask t2=(GvDownloadTask) c.findById(id);
					 	OperLogUtil4vod.log("优先任务:"+t2.getMtrl().getMaterialDescription());
					} catch (Exception e) {

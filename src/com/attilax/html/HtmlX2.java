...
import com.attilax.cc.Ibalance;
import com.attilax.cc.hsbc;
import com.attilax.cc.item;
...
it.fromFile=frompath;
	...
	public Element getTable(Document doc, String keyword,String paichuWord) {
		...
		for (Element e : el) {
			...
			if (tabletxt.trim().contains(keyword)) {
				System.out.println("-----------");
				System.out.println("--tab txt:"+tabletxt);
				tb_mid = e;
				break;
			}
			// tabletxt=e.toString();
	...
	public static float trimBalanceMinipay_o3(String bls_str) {
		 ...
		 */
	public Element getTr(Element tab, int idx,boolean includeTbody) {
	//	Element tab=tabs.get(0);
		Element tbody=	tab.children().get(0);
		Element tr=tbody.children().get(idx);
		return tr;
	}
}

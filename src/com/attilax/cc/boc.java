...
public class boc  extends ccParser{
	...
	public Logger logger = Logger.getLogger(boc.class.getName());
	/**
	 * @param args
	 * @throws IOException 
	 */
	public static void main(String[] args) throws IOException {
		...
		String path = "c:\\boc.eml";
	 path="c:\\ccx\\cc.cb\\中国银行银行卡电子账单.eml";
	 path="c:\\ccx\\cc.cb\\中国银行银行卡电子账单(14).eml";
	 boc c=new boc();
 ...
 */
	...
	//	if(1==1)return null;
		...
		for (Element e : trs) {
			...
			FileService.writeFile("c:\\tds.txt", tds.toString());
	...
	private String getCardNo(Document doc) {
		String html2txt = htmlx.html2txt(doc.toString());
		html2txt=html2txt.replaceAll("\r", "");
		html2txt=html2txt.replaceAll("\n", "");
		String s=	strUtil.getMidtrings( html2txt  ,"信用卡号","账单日");
		
		
	return s;
}
	public boolean paichuLine(String string) {
		String[] a=paichuLIne.split(",");
		for(String s:a)
	...
	public boolean paichuLinex(String string,String paichuLIneKeywords_str) {
		String[] a=paichuLIneKeywords_str.split(",");
		for(String s:a)
		{	if(string.contains(s))
		     return true;
...
s2=doc.text();
	...
	public item getItem(Elements tds, String yearMonth) {
		...
		for (Element td : tds) {
			...
			if(tds.size()==6)
			{
				...
				{
					String t2=numUtil.trim(text);  	if(t2.equals(""))t2="0";
	...
	public static  void mainx(String[] string) {
//		String s2="   账单周期 Statement cycle 2013/11/15 - 2013/12/14 综合信用额度";
//		
//		String abcdef="abcdef";
//		String fd="ab(.*?)ef";

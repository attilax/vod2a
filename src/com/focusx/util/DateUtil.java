...
public class DateUtil {
	...
	public static SimpleDateFormat sdf_YM_China = new SimpleDateFormat(
			"yyyy年MM月");
	public static SimpleDateFormat sdf_YMd_China = new SimpleDateFormat(
			"yyyy年MM月dd日");
	...
	// 根据两个时间、分页数获取月
	public static List<String> getMonths(String starTime, String endTime,
		...
		try {
			...
			months.add(starTime);
			months_map.put(starTime,
					starTime + " 00:00:00" + "=" + map.get("last"));
			if (count_other == 0) {// 表示两个日期在同一个月中
				...
				months_map.put(endTime, map_end.get("first") + "=" + endTime
						+ " 23:59:59");
				months.add(endTime);
				count = 2;
			} else {
	...
	// 根据两个时间、分页数获取周
	public static List<String> getWeeks(String lastDay, int pageNumber,
			int pageSize) {
		...
		try {
			...
			for (int i = 1; i < count_other + 1; i++) {
				list.add(getLastWeek_Day(mon2, i * 7) + "="
						+ getLastWeek_Day(sun2, i * 7));
			}
	...
	// 根据两个时间、分页数获取日
	public static List<String> getDays(String starTime, String endTime,
			int pageNumber, int pageSize) {
